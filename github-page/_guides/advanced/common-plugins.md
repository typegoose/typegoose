...
Typegoose has the prop option `autopopulate` implemented, but it only has an effect if
[`mongoose-autopopulate`](https://github.com/mongodb-js/mongoose-autopopulate) is installed and used too.
...
**PLEASE NOTE:** If you have a 'ref' which refers back to its own class/ model, like having a User class with a `createdBy` field referring
...
The Typegoose project provides an [`auto-increment` plugin](https://github.com/typegoose/auto-increment) for Mongoose. Here is how to use
it:
...
Only increases the field if the document is _new_ and the counter is stored in a counter-collection (default field: `_id`).
