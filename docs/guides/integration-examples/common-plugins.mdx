...
Typegoose can be used together with `mongoose-paginate-v2`'s types.

Though actual usage requires some manual type picking, due to `mongoose-paginate-v2` not providing anything other than a `mongoose.Model` wrapper, which is not directly comaptible with typegoose's types:
...
(async () => {
  ...
  // types are mostly available
  const result = await UserModel.paginate({ username: 'hello' }, { page: 0, limit: 10 });
...
:::note
Please note that as of `mongoose-paginate-v2@1.9.1` types for `FilterQuery` do not work, likely due to not being updated for mongoose 9.0.

See [mongoose-paginate-v2#239](https://github.com/aravindnc/mongoose-paginate-v2/issues/239)
:::
